 ...
 *  - Calls its own backend endpoint  GET /api/regional/validator/incidents
 *  - Sends decisions via             PATCH /api/regional/incidents/:id/verification
...
export default function ValidatorDashboard() {
  ...
  const fetchQueue = useCallback(async () => {
    ...
    try {
      const data: QueueResponse = await apiFetch(
        `/api/regional/validator/incidents?${params.toString()}`
      );
  ...
  const submitAction = async () => {
    ...
    try {
      await apiFetch(
        `/api/regional/incidents/${actionTarget.incident_id}/verification`,
        {
          method: "PATCH",
          body: JSON.stringify({
            action: actionType,
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap gap-4 mb-6">
        ...
        >
          <option value="">All statuses (default queue)</option>
