...
from services.analytics_read_model import get_analyst_export_rows, get_export_rows, get_incident_export_data
...
def _write_afor_pdf(path: str, data: dict[str, Any]) -> None:
    ...
    story.append(Paragraph(f"<b>Incident ID:</b> {incident_id}  |  <b>Reference:</b> {data.get('reference_number', 'N/A')}  |  <b>Date:</b> {data.get('date_notification', 'N/A')}", styles["Normal"]))
    ...
    a_data = [
        ...
        ["Municipality", data.get("municipality", ""), "Fire Origin / Address", data.get("address_fire", "")],
        ["Date Notified", data.get("date_notification", ""), "Time Notified", data.get("time_notification", "")],
        ["Engine Dispatched", data.get("engine_dispatched", ""), "Time Dispatched", data.get("time_dispatched", "")],
        ["Time Arrived", data.get("time_arrived", ""), "Response Time (min)", data.get("response_time_min", "")],
        ["Distance (km)", data.get("distance_km", ""), "Highest Alarm", data.get("highest_alarm", "")],
        ["Gas Consumed (L)", data.get("gas_consumed_liters", ""), "Responder Type", data.get("responder_type", "")],
    ]
    ...
    story.append(Spacer(1, 4))
    b_data = [
        ["Classification", data.get("general_category", ""), "Sub-Category", data.get("sub_category", "")],
        ["Area of Origin", data.get("area_of_origin", ""), "Stage of Fire", data.get("stage_of_fire", "")],
        ["Extent of Damage", data.get("extent_of_damage", ""), "Est. Damage (PHP)", data.get("estimated_damage", "")],
        ["Structures Affected", data.get("structures_affected", ""), "Households Affected", data.get("households_affected", "")],
        ["Families Affected", data.get("families_affected", ""), "Individuals Affected", data.get("individuals_affected", "")],
        ["Vehicles Affected", data.get("vehicles_affected", ""), "", ""],
        ["Floor Area (sqm)", data.get("extent_floor_area", ""), "Land Area (ha)", data.get("extent_land_area", "")],
    ]
    ...
    story.append(Spacer(1, 4))
    c_data = [
        ["BFP Fire Trucks", data.get("bfp_fire_trucks", ""), "BFP Manned Trucks", data.get("bfp_manned_trucks", "")],
        ["Non-BFP Fire Trucks", data.get("non_bfp_trucks", ""), "BFP Ambulance", data.get("bfp_ambulance", "")],
        ["Non-BFP Ambulance", data.get("non_bfp_ambulance", ""), "BFP Rescue Trucks", data.get("bfp_rescue_trucks", "")],
        ["Non-BFP Rescue Trucks", data.get("non_bfp_rescue_trucks", ""), "Other Vehicles", data.get("other_vehicles", "")],
    ...
    e_data = [
        ["", "Male", "Female"],
        ["Injured Civilian", data.get("civilian_injured_male", ""), data.get("civilian_injured_female", "")],
        ["Injured BFP Firefighter", data.get("firefighter_injured_male", ""), data.get("firefighter_injured_female", "")],
        ["Civilian Fatality", data.get("civilian_deaths_male", ""), data.get("civilian_deaths_female", "")],
        ["BFP Firefighter Fatality", data.get("firefighter_deaths_male", ""), data.get("firefighter_deaths_female", "")],
    ]
...
def export_analyst_incidents_task(
    ...
    if export_mode == "afor" and incident_id:
        ...
        writers = {
            ...
            "excel":("xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", _write_afor_excel),
        }
